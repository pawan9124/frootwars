...
class LevelData {
  constructor() {
    this.levels = [
      {gapX:5, gapY:30, widthDiff: 0, total: 5, coinChance:0.4, enemyChance:0.2 },
...
class Platform extends GameObject {
  ...
  setClippingWidth(width) {
    this.graphic.instance.mask = new createjs.Shape(new 
      createjs.Graphics().beginFill('#000').drawRect(0,0,width,this.getBounds().height));
...
class World extends createjs.Container {
  ...
  generateEnemies() {
      ...
      var chance = this.levelData.levels[levelNumber].enemyChance;
  		...
  		if(Math.random() < chance) { // ~30% chance
  			var enemy = new Enemy();
  ...
  generateCoins() {
      ...
      var chance = this.levelData.levels[levelNumber].coinChance;
  		...
  		if (Math.random() < chance) { //~80% chance
  			var coin = new Coin();
